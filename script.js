...
// ---- RARETÉS ----
// Les documents anciens (manuscrits médiévaux, cartes) sont plus rares
const RARITY_BOOST = {
  "manuscrit": 2, "carte": 1.5,
};
...
function pickRarity(docType) {
  // Les manuscrits et cartes ont plus de chance d'être rares
  const boost = RARITY_BOOST[docType] || 1;
  const shifted = RARITIES.map((r, i) => ({
    ...r,
    weight: i >= 2 ? r.weight * boost : r.weight / boost
  }));
...
function showModal(card) {
  ...
  div.innerHTML = `
    <style>
      ...
      .gc-theme {
        font-size: 10px; color: #d4af3799;
        letter-spacing: 1px; margin-top: 2px;
      }
      .gc-img {
        background: #1a1a2e; padding: 16px; text-align: center;
      }
      .gc-img img {
    ...
    <div class="gc">
      <div class="gc-head">
        <div class="gc-badge">${rarity.label}</div>
...
function showCollection() {
  ...
  container.innerHTML = coll.map(card => `
    <div onclick="window.open('${card.url || card.itemUrl}','_blank')"
      ...
      <div style="padding:0 6px 6px;font-size:9px;color:#888;">${card.year || "Date inconnue"}</div>
    </div>
  `).join('');
}
